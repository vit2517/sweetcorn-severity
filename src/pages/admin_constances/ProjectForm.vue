                      ...
                      <div class="row">
                        <div class="col-md-12 col-xs-12 q-pa-xs">
                          <q-input
                            ...
                            standout
                            bottom-slots
                            v-model="individual.project_name"
                            label="โครงการ *"
                            clearable
                            :rules="[
                              (val) =>
                        ...
                        <template v-slot:body-cell-actions="props">
                          <q-td :props="props">
                            <q-btn
                              icon="mode_edit"
                              label="แก้ไข"
                              @click="editUser(props.row.project_id)"
                            ></q-btn>
                            <q-btn
                              icon="delete"
                              label="ลบ"
...
export default {
  ...
  data() {
    return {
      ...
      isEdit: false,
      status: "บันทึก",
      visibleColumns: ref(["actions", "project_id", "project_name"]),
      columns: [
    ...
    submitForm() {
      if (!this.isEdit) {
        ...
        this.$q
          ...
          .onOk(() => {
            console.log(
              "บันทึกการแก้ไข project_id:",
              this.individual.project_id
            );
            axios
              .post(this.url_api_individual, {
                action: "update",
                project_id: this.individual.project_id,
                project_name: this.individual.project_name,
              })
              .then((response) => {
    ...
    editUser(project_id) {
      ...
      axios
        ...
        .then(function (response) {
          ...
          self.individual.project_id = response.data.project_id;
          self.individual.project_name = response.data.project_name;
        })
        .catch(function (error) {
          console.log(error);
        });
  ...
  created() {
    ...
    var folder = "icp_v1_admin_constances/ProjectForm/api-individual.php";
    if (!www) {
