                      ...
                      <div class="row">
                        <div class="col-md-9 col-xs-12 q-pa-xs">
                          <q-input
                            ...
                            standout
                            bottom-slots
                            v-model="individual.importance_name"
                            label="ความสำคัญ *"
                            clearable
                            :rules="[
                              (val) =>
                                (val && val.length > 0) || 'ต้องใส่ความสำคัญ',
                            ]"
                          >
                        ...
                        <div class="col-md-3 col-xs-12 q-pa-xs">
                          <q-input
                            ...
                            standout
                            bottom-slots
                            v-model="individual.importance_value"
                            label="ค่าความสำคัญ *"
                            clearable
                            mask="#"
                            fill-mask
                        ...
                        <template v-slot:body-cell-actions="props">
                          <q-td :props="props">
                            <q-btn
                              icon="mode_edit"
                              label="แก้ไข"
                              @click="editUser(props.row.importance_id)"
                            ></q-btn>
                            <q-btn
                              icon="delete"
                              label="ลบ"
                              @click="
                                deleteUser(
                                  props.row.importance_id,
                                  props.row.importance_name
                                )
                              "
                            ></q-btn>
                          </q-td>
...
export default {
      ...
      if (!this.isEdit) {
        this.$q
          ...
          .onOk(() => {
            ...
            const newindividual = {
              importance_name: this.individual.importance_name,
              importance_value: this.individual.importance_value,
            };
        ...
        this.$q
          ...
          .onOk(() => {
            console.log(
              "บันทึกการแก้ไข perform_id:",
              this.individual.importance_id
            );
            axios
              .post(this.url_api_individual, {
                action: "update",
                importance_id: this.individual.importance_id,
                importance_name: this.individual.importance_name,
                importance_value: this.individual.importance_value,
              })
              .then((response) => {
    ...
    editUser(importance_id) {
      ...
      axios
        ...
        .then(function (response) {
          ...
          self.individual.importance_id = response.data.importance_id;
          self.individual.importance_name = response.data.importance_name;
          self.individual.importance_value = response.data.importance_value;
        })
        .catch(function (error) {
          console.log(error);
        });
    },
    deleteUser(importance_id, importance_name) {
      this.$q
        .dialog({
          title: "ยืนยัน",
          message:
  ...
  created() {
    ...
    var folder = "icp_v1_admin_constances/ImportanceForm/";
