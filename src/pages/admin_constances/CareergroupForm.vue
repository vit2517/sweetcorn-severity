                      ...
                      <div class="row">
                        <div class="col-md-12 col-xs-12 q-pa-xs">
                          <q-input
                            ...
                            standout
                            bottom-slots
                            v-model="individual.ca_group_name"
                            label="กลุ่มอาชีพ *"
                            clearable
                            :rules="[
                              (val) =>
                        ...
                        <template v-slot:body-cell-actions="props">
                          <q-td :props="props">
                            <q-btn
                              icon="mode_edit"
                              label="แก้ไข"
                              @click="editUser(props.row.career_group_id)"
                            ></q-btn>
                            <q-btn
                              icon="delete"
                              label="ลบ"
...
export default {
  ...
  data() {
    return {
      ...
      isEdit: false,
      status: "บันทึก",
      visibleColumns: ref(["actions", "career_group_id ", "ca_group_name"]),
      columns: [
  ...
  methods: {
    ...
    resetForm() {
      ...
      // ข้อมูลส่วนตัว
      this.individual.career_group_id = "";
      this.individual.ca_group_name = "";
    },
    submitForm() {
      if (!this.isEdit) {
        this.$q
          ...
          .onOk(() => {
            console.log(
              "บันทึกข้อมูล ca_group_name:",
              this.individual.ca_group_name
            );
            const newindividual = {
              ca_group_name: this.individual.ca_group_name,
            };
        ...
        this.$q
          ...
          .onOk(() => {
            console.log(
              "บันทึกการแก้ไข career_group_id:",
              this.individual.career_group_id
            );
            axios
              .post(this.url_api_individual, {
                action: "update",
                career_group_id: this.individual.career_group_id,
                ca_group_name: this.individual.ca_group_name,
              })
              .then((response) => {
    ...
    editUser(career_group_id) {
      ...
      axios
        ...
        .then(function (response) {
          ...
          self.individual.career_group_id = response.data.career_group_id;
          self.individual.ca_group_name = response.data.ca_group_name;
        })
        .catch(function (error) {
          console.log(error);
        });
    },
    deleteUser(career_group_id, ca_group_name) {
      this.$q
        .dialog({
          title: "ยืนยัน",
          message:
  ...
  created() {
    ...
    var folder = "icp_v1_admin_constances/Career_group/api-individual.php";
    if (!www) {
