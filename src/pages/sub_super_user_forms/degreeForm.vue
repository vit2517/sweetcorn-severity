                      ...
                      <div class="row">
                        ...
                        <div class="col-md-12 col-xs-12 q-pa-xs">
                          <q-input
                            ...
                            standout
                            bottom-slots
                            v-model="individual.degree_name"
                            label="ปริญญา *"
                            clearable
                            :rules="[
                              (val) =>
                        ...
                        <template v-slot:body-cell-actions="props">
                          <q-td :props="props">
                            <q-btn
                              icon="mode_edit"
                              label="แก้ไข"
                              @click="editUser(props.row.degree_id)"
                            ></q-btn>
                            <q-btn
                              icon="delete"
                              label="ลบ"
...
export default {
  ...
  data() {
    return {
      ...
      url_api_member: "",
      // ------------------------------------------------------------------------------
      title: "ข้อมูลปริญญา(ผู้ดูแลกลุ่ม)",
      email: "",
      username: "",
      password: "",
      repassword: "",
      ...
      if (!this.isEdit) {
        this.$q
          ...
          .onOk(() => {
            ...
            console.log(
              "บันทึกข้อมูล institute_id:",
              this.individual.institute_id
            );
            console.log("บันทึกข้อมูล faculty_id:", this.individual.faculty_id);
            console.log(
              "บันทึกข้อมูล degree_name:",
              this.individual.degree_name
            );
            const newindividual = {
              institute_id: this.individual.institute_id,
              faculty_id: this.individual.faculty_id,
        ...
        this.$q
          ...
          .onOk(() => {
            console.log("บันทึกการแก้ไข degree_id:", this.individual.degree_id);
            axios
              .post(this.url_api_individual, {
                action: "update",
                institute_id: this.individual.institute_id,
    ...
    editUser(degree_id) {
      ...
      axios
        ...
        .then(function (response) {
          ...
          self.individual.degree_name = response.data.degree_name;
        })
        .catch(function (error) {
          console.log(error);
        });
    },
    deleteUser(degree_id, degree_name) {
      this.$q
        .dialog({
          title: "ยืนยัน",
          message:
            "คุณต้องการลบ [ " + degree_id + "-" + degree_name + " ] หรือไม่ ?",
          cancel: true,
          persistent: true,
        })
        .onOk(() => {
  ...
  created() {
    ...
    var folder = "icp_v1_sub_admin/degree/";
